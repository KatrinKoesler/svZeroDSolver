...
void ValveSpline::update_solution(
  ...
  double Rmin = parameters[global_param_ids[ParamId::RMIN]];
  double Rmax = parameters[global_param_ids[ParamId::RMAX]];
  double steep = parameters[global_param_ids[ParamId::EPSILON]];
...
double ValveSpline::calculate_beta(double p, double p_open, double epsilon, double R_max, double R_min) {
  
  double s = (p - p_open + 0.5 * epsilon) / epsilon;
  ...
  if (p > p_open - 0.5 * epsilon) {
    return (p - p_open)/R_max;
  }
  else if (p >= p_open - 0.5 * epsilon && p < p_open + 0.5 * epsilon) {
    return h_00 * p_0 + h_10 * m_0 * epsilon + h_01 * p_1 + h_11 * m_1 * epsilon;
  }
  else if (p >= p_open + 0.5 * epsilon) {
    return (p - p_open)/R_min;
  }
}
